...
@Component({
  ...
  template: `
    <div class="medias-wrap">
      ...
      <div class="app-card upload-card">
        ...
        <!-- Options visibles seulement si un fichier est sélectionné -->
  ...
  styles: [`
    ...
    /* Drop zone */
    .drop-zone {
      border: 2px dashed var(--border, rgba(45,106,79,.25));
      border-radius: 16px;
      min-height: 180px;
      display: flex;
    ...
    .link-btn {
      ...
      color: var(--g700, #3d7a5c); font-weight: 600; cursor: pointer;
    ...
    .count {
      ...
      background: var(--g100, #d4e8da); color: var(--g800, #2d6a4f);
      border-radius: 20px; padding: 1px 10px; font-size: .82rem;
      font-weight: 700; margin-left: 8px;
    }
    .filters { display: flex; gap: 8px; flex-wrap: wrap; }
    .filter-btn {
      padding: 6px 16px; border-radius: 20px; border: 1.5px solid var(--border, rgba(45,106,79,.2));
      background: white; color: var(--g700, #3d7a5c); font-size: .85rem;
      font-weight: 600; cursor: pointer; transition: all .15s;
    }
    .filter-btn:hover { border-color: var(--g700, #3d7a5c); }
    .filter-btn.active {
      background: var(--g700, #3d7a5c); color: white; border-color: var(--g700, #3d7a5c);
    }
    ...
    .legende {
      font-size: .82rem; color: var(--text-muted, #556b5a);
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin: 0;
    }
    .del-btn {
      flex-shrink: 0;
      background: none; border: 1.5px solid transparent; color: #c0392b;
    ...
    .empty-state {
      ...
      color: var(--text-muted, #556b5a); font-size: .95rem;
    }
...
export class MediasAdminComponent implements OnInit {
  ...
  handleFile(file: File) {
    ...
    if (!file.type.startsWith('image/')) {
      this.errorMsg = 'Ce fichier n\'est pas une image.';
      return;
    }
    if (file.size > 10 * 1024 * 1024) {
      this.errorMsg = 'Le fichier dépasse la limite de 10 Mo.';
      return;
    }
  ...
  onUpload() {
    ...
    this.dataService.uploadMedia(this.selectedFile, this.selectedCategorie, this.legende).subscribe({
      next: () => {
        ...
        this.successMsg = 'Photo ajoutée avec succès.';
        this.clearFile();
        this.load();
      },
      error: (err) => {
        ...
        if (err.status === 401 || err.status === 403) {
          this.errorMsg = 'Accès refusé (401/403). Veuillez vous reconnecter.';
        } else if (err.status === 0) {
          this.errorMsg = 'Impossible de contacter le serveur. Vérifiez que le backend est démarré.';
        } else if (err.error && typeof err.error === 'string') {
          this.errorMsg = `Erreur ${err.status} : ${err.error}`;
        } else {
          this.errorMsg = `Erreur ${err.status} lors de l'envoi. Consultez les logs du backend.`;
        }
